...
#include <ordgeo/embed/cmp.hpp>
#include <ordgeo/embed/dlib_opt.hpp>
#include <ordgeo/linalg/linalg.hpp>

using Eigen::Index;
using Eigen::MatrixXd;
using Eigen::MatrixXi;
using Eigen::VectorXd;
using ORDGEO_NAMESPACE::linalg::dist;
...
struct SOEBase {
	SOEBase(Iter consBegin, Iter consEnd, std::function<double(size_t)> weights,
		double scale)
		: consBegin(consBegin), consEnd(consEnd), weights(weights), scale(scale)
	{
	}
